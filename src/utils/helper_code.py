...
def load_signals(record):
    ...
    signal_files_exist = all(
        os.path.isfile(os.path.join(path, signal_file)) for signal_file in signal_files
    )
...
def get_variables(string, variable_name, sep=","):
    ...
    for l in string.split("\n"):
        if l.startswith(variable_name):
            variables += [
                variable.strip()
                for variable in l[len(variable_name) :].strip().split(sep)
            ]
            has_variable = True
...
def get_labels_from_header(string):
    ...
    if not has_labels:
        raise Exception(
            "No labels available: are you trying to load the labels from the held-out data, or did you forget to prepare the data to include the labels?"
        )
    return labels
...
def convert_signal(x, num_quant_levels, min_amp, max_amp, max_t):
    ...
    y = x[idx]
    y = np.round((num_quant_levels - 1) * (y - min_amp) / (max_amp - min_amp) + 1).astype(
        int
    )
...
def compute_asci_metric(x_ref, x_est, beta=0.05, keep_nans=True):
    ...
    if beta <= 0 or beta > 1:
        raise ValueError(
            "The beta value should be greater than 0 and less than or equal to 1."
        )
...
def compute_weighted_absolute_difference(
    x_ref, x_est, sampling_frequency, keep_nans=True
):
    # Check the reference and estimated signals.
    x_ref = np.asarray(x_ref).copy()
    x_est = np.asarray(x_est).copy()
