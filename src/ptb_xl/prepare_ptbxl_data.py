...
def get_parser():
    ...
    parser.add_argument(
        "-pd", "--ptbxl_database_file", type=str, required=True
    )  # ptbxl_database.csv
    parser.add_argument(
        "-pm", "--ptbxl_mapping_file", type=str, required=True
    )  # scp_statements.csv
    parser.add_argument(
        "-sd", "--sl_database_file", type=str, required=True
    )  # 12sl_statements.csv
    parser.add_argument(
        "-sm", "--sl_mapping_file", type=str, required=True
    )  # 12slv23ToSNOMED.csv
...
def run(args):
    ...
    df_ptbxl_database.scp_codes = df_ptbxl_database.scp_codes.apply(
        lambda x: ast.literal_eval(x)
    )

    # Map the PTB-XL classes to superclasses.
    df_ptbxl_database["diagnostic_superclass"] = df_ptbxl_database.scp_codes.apply(
        assign_superclass
    )
    ...
    acute_mi_statements = set(
        [821, 822, 823, 827, 829, 902, 903, 904, 963, 964, 965, 966, 967, 968]
    )
    ...
    for record in tqdm(records):
        ...
        scp_codes = [
            scp_code for scp_code in scp_code_dict if scp_code_dict[scp_code] >= 0
        ]
        ...
        if os.path.normpath(args.input_folder) != os.path.normpath(args.output_folder):
            ...
            for signal_file in signal_files:
                input_signal_file = os.path.join(
                    args.input_folder, relative_path, signal_file
                )
                output_signal_file = os.path.join(
                    args.output_folder, relative_path, signal_file
                )
