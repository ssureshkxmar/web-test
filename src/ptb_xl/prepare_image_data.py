...
def run(args):
    ...
    for record in tqdm(records):
        ...
        record_suffixes = [
            os.path.splitext(image_file)[0].split("-")[-1]
            for image_file in record_image_files
        ]
        if all(is_number(suffix) for suffix in record_suffixes):
            record_image_files = sorted(
                record_image_files,
                key=lambda image_file: float(
                    os.path.splitext(image_file)[0].split("-")[-1]
                ),
            )
        else:
        ...
        if os.path.normpath(args.input_folder) != os.path.normpath(args.output_folder):
            ...
            for image_file in image_files:
                input_image_file = os.path.join(
                    args.input_folder, relative_path, image_file
                )
                output_image_file = os.path.join(
                    args.output_folder, relative_path, image_file
                )
